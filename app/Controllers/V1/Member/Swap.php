...
class Swap extends BaseController
{
	...
	public function getSummary(){
        ...
        $balance = $this->wallet->get_balance($data->userid,$data->source);
        $fee = $this->getFee($data->source);
        $amount_swap = $data->amount - $fee["fee"] - $fee["cost"];
    ...
    private function getFee($currency) {
        ...
        return array("fee" => IS_NULL($feedb->swap) ? 0 : $feedb->swap, "cost" => IS_NULL($costdb->swap) ? 0 : $costdb->swap);
    }	
